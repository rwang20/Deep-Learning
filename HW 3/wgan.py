...
Original file is located at
    https://colab.research.google.com/drive/1ZNAe383Pq43Fbph1zQPxaYxkxBTQrItw
"""
...
os.makedirs('/content/drive/My Drive/WGAN', exist_ok=True)
os.makedirs('/content/drive/My Drive/WGAN/train_images', exist_ok=True)
os.makedirs('/content/drive/My Drive/WGAN/results', exist_ok=True)
os.makedirs('/content/drive/My Drive/WGAN/trainLog', exist_ok=True)
...
def generator_model():
    """
    Create and return a generator model
...
def write_to_tensorboard(generator_step, summary_writer, losses):

    summary = tf.Summary()

    value = summary.value.add()
    value.simple_value = losses[1]
    value.tag = 'Discriminator Real Loss'

    value = summary.value.add()
    value.simple_value = losses[2]
    value.tag = 'Discriminator Fake Loss'
    ...
    value.tag = 'Discriminator Loss (D_real - D_fake)'

    value = summary.value.add()
    value.simple_value = losses[1] + losses[2]
    value.tag = 'Discriminator Loss (D_fake + D_real)'
...
def train_WGAN():
    ...
    if os.path.exists('/content/drive/My Drive/DCGAN/model/best_GAN'):
        ...
        GAN, generator, discriminator = WGAN_model()
    ...
    for iters in range(MAX_ITER):
        ...
        GAN.train_on_batch(latent_samples, Y_train_fake)

        # Generate latent samples
        latent_samples = generate_latent_samples(TRAIN_BATCH_SIZE, 100)
        g_loss = GAN.train_on_batch(latent_samples, Y_eval_fake)
        ...
        if (iters + 1) % SAVE_PERIOD == 0:
            ...
            # tym,kgenerator_loss_batch = GAN.train_on_batch(latent_samples, Y_eval_real)
            ...
            with open('/content/drive/My Drive/WGAN/losses.txt', "a") as file:
                file.write("DIS_LOSS_BATCH: {}, GEN_LOSS_BATCH: {}") 
                file.write('\n')
            ...
            img.save(os.path.join('/content/drive/My Drive/WGAN/train_images', 'generated_img_' + str(iters + 1) + '.png'))
            ...
            with open('/content/drive/My Drive/WGAN/fid_scores.txt', "a") as file:
            ...
            if cur_fid_score < best_fid_score:
                ...
                with open('/content/drive/My Drive/WGAN/best_fid.txt', "w") as file:
                    file.write(str(cur_fid_score))

                # Save the Generator model
                if os.path.exists('/content/drive/My Drive/WGAN/model/best_generator'):
                    shutil.rmtree('/content/drive/My Drive/WGAN/model/best_generator')

                generator.save('/content/drive/My Drive/WGAN/model/best_generator')

                # Save the Discriminator model
                if os.path.exists('/content/drive/My Drive/WGAN/model/best_discriminator'):
                    shutil.rmtree('/content/drive/My Drive/WGAN/model/best_discriminator')

                discriminator.save('/content/drive/My Drive/WGAN/model/best_discriminator')

                # Save the GAN model
                if os.path.exists('/content/drive/My Drive/WGAN/model/best_GAN'):
                    shutil.rmtree('/content/drive/My Drive/WGAN/model/best_GAN')

                GAN.save('/content/drive/My Drive/WGAN/model/best_GAN')
    return
...
generator = load_model('/content/drive/My Drive/WGAN/model/best_generator') 
